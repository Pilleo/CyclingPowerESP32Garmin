...
void simulateCadencePulse(uint32_t durationMs) {
  // Note: computer->update() no longer polls, but we call it to trigger BLE
  // updates/calculations. The ISRs are triggered immediately by setPinState.

  mockSys.setPinState(PIN_CADENCE,
                      false); // Magnet passes (Falling Edge -> ISR Triggered)
...
void test_system_sleeps_after_timeout() {
  ...
  TEST_ASSERT_TRUE_MESSAGE(mockSys.wasSleepCalled,
                           "System should have entered deep sleep");
