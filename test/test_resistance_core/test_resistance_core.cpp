...
const ResistanceLevelPins DUMMY_PINS = {
    .backwards = 0, .limit = 0, .position = 0, .forwards = 0};
...
void tearDown(void) { delete resCore; }

/**
 * Test: Pure Logic Increment
 * Rationale: Verify counter increases when forward flags are set, without using
 * digitalRead.
 */
...
void test_res_core_debounce() {
  ...
  // Attempt Bounce (Delta 1ms)
  resCore->onPositionPulse(lastTime + 1, true, false);
